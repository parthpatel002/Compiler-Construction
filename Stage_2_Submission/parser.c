			...
			if(lookahead == p->symType.terminalType) {
                ...
                *errors = 1;
                ...
                if(p->symType.terminalType == TK_SEM) {
                    ...
                    sprintf(syntax_error_text[error_index++], "Line %d: Semicolon is missing\n", new_token->line_no);
                }
                else {
                    ...
                    sprintf(syntax_error_text[error_index++],"Line %d: The token %s for lexeme %s  does not match with the expected token %s\n", new_token->line_no, tokenMap[lookahead], new_token->lexeme, tokenMap[p->symType.terminalType]);
                }
            }
        }
        //if top of the stack is a non terminal
            ...
            if(rule_no == -1) {
                ...
                *errors = 1;
                ...
                sprintf(syntax_error_text[error_index++], "Line %d: The token of type %s for lexeme %s does not match with the expected token of type %s\n", new_token->line_no, tokenMap[new_token->type], new_token->lexeme, non_terminals[p->symType.nonTerminalType]);
            }
            else {
